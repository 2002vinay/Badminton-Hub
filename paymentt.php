...
//     $amount = mysqli_real_escape_string($conn, $total);
    ...
    // $payment = "INSERT INTO payment(fname, lname, email, address, pin, amount) VALUES('$fname', '$lname', '$email', '$address', '$pin', '$amount')";
    // mysqli_query($conn, $payment);
    //     $error = '<div class="alert alert-success" role="alert">Payment successful!</div>';
    //     header('location: order.php');
    //     exit;
    // } else {
    //     $error = '<div class="alert alert-danger" role="alert">Payment failed: ' . mysqli_error($conn) . '</div>';
    // }
...
if (isset($_POST['pay'])) {
  ...
  for ($i = 0; $i < $count; $i++) {
    ...
    // Inserting into table
    $payment = "INSERT INTO payment1 (fname, lname, email, address, pin, amount,o_date,pname,pcost,pquantity) VALUES ('$fname', '$lname', '$email', '$address', '$pin', '$amount','$currentdate','$name','$cost','$quantity')";
    mysqli_query($conn, $payment);
...
while($data = mysqli_fetch_assoc($result)) {
    echo '<input type="hidden" name="n[]" value="' . $data['pname'] . '">';
    echo '<input type="hidden" name="q[]" value="' . $data['pquantity'] . '">';
}
?>
